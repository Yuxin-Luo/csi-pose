...
/* WiFi 任务上下文 — info/buf 在 return 时释放, 必须用队列复制 */
static void csi_cb(void *ctx, wifi_csi_info_t *info)
{
    ...
    if (s_st.pay_off < 0) {
        ...
        s_st.magic_reject++; /* 其他流量的 CSI —— 保持锁定 */
        return;
    }
    ...
    if (!s_mac_set[t]) {
        ...
        s_mac_set[t] = true;
        s_mac_dirty[t] = true; /* NVS 写入在 framer 任务中进行 (避免 flash 阻塞) */
    } else if (memcmp(s_mac[t], info->mac, 6) != 0) {
        s_st.mac_reject++; /* tx_idx 伪装 / 重复板检测 */
        return;
    }
...
esp_err_t csi_rx_init(QueueHandle_t q)
{
    s_q = q;
    for (int i = 0; i < 3; i++)
        s_mac_set[i] = csil_cfg_get_mac(i, s_mac[i]); /* 从 NVS 恢复 pin */

    ESP_ERROR_CHECK(csil_espnow_rx_init());
    wifi_csi_config_t cc = {
        .lltf_en = true,        /* first_word_invalid 隔离到 LLTF 区 */
        .htltf_en = true,
        .stbc_htltf2_en = true,
        .ltf_merge_en = true,   /* 显式开 —— M0 实测 dump 确认 ±27/28 行为 */
